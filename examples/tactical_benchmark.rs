...
use kingfisher::mcts::{mcts_pesto_search, tactical_mcts_search, TacticalMctsConfig};
...
impl BenchmarkResults {
    pub fn analyze_efficiency(&self) {
        ...
        let tactical_avg_nn = self
            .tactical_mcts
            .iter()
            .map(|r| r.nn_evaluations as f64)
            .sum::<f64>()
            / self.tactical_mcts.len() as f64;

        let classical_avg_nn = self
            .classical_mcts
            .iter()
            .map(|r| r.nn_evaluations as f64)
            .sum::<f64>()
            / self.classical_mcts.len() as f64;
        ...
        println!(
            "   Tactical-Enhanced MCTS: {:.1} avg NN calls",
            tactical_avg_nn
        );
        println!(
            "   Classical MCTS:         {:.1} avg NN calls",
            classical_avg_nn
        );
        println!("   Reduction:              {:.1}%", nn_reduction);

        // Calculate tactical move exploration
        let tactical_moves_explored: u32 = self
            .tactical_mcts
            .iter()
        ...
        println!(
            "   Total tactical moves explored: {}",
            tactical_moves_explored
        );
        println!(
            "   Average per position: {:.1}",
        ...
        println!(
            "   Alpha-Beta:             {} mates found",
            alpha_beta_mates
        );

        // Calculate search time efficiency
        let tactical_avg_time = self
            .tactical_mcts
            .iter()
        ...
        let alpha_beta_avg_time = self
            .alpha_beta
            .iter()
            .map(|r| r.search_time.as_millis() as f64)
            .sum::<f64>()
            / self.alpha_beta.len() as f64;
        ...
        println!("\n🌳 Node Efficiency:");
        println!(
            "   Tactical-Enhanced MCTS: {:.0} avg nodes",
            tactical_avg_nodes
        );
        println!(
            "   Classical MCTS:         {:.0} avg nodes",
            classical_avg_nodes
        );
    ...
    pub fn print_detailed_results(&self) {
        ...
        for (i, pos) in self.test_positions.iter().enumerate() {
            ...
            if i < self.tactical_mcts.len() {
                ...
                println!(
                    "   Tactical MCTS:  {:?} ({}ms, {} NN calls, {} tactical)",
                    tactical.best_move,
                    tactical.search_time.as_millis(),
                    tactical.nn_evaluations,
                    tactical.tactical_moves_explored
                );
                println!(
                    "   Classical MCTS: {:?} ({}ms, {} NN calls)",
                    classical.best_move,
                    classical.search_time.as_millis(),
                    classical.nn_evaluations
                );
                println!(
                    "   Alpha-Beta:     {:?} ({}ms)",
                    alpha_beta.best_move,
                    alpha_beta.search_time.as_millis()
                );
            }
        }
    }
...
fn benchmark_tactical_mcts(
    ...
    let (best_move, stats, _root) = tactical_mcts_search(board.clone(), move_gen, tactical_config);
