...
use kingfisher::mcts::tactical::{
    clear_tactical_cache, get_tactical_cache_stats, identify_tactical_moves,
};
...
fn benchmark_cache_performance(positions: &[(&str, &str)], move_gen: &MoveGen) {
    println!(
        "Testing cache performance with {} positions:",
        positions.len()
    );
...
fn benchmark_repeated_positions(positions: &[(&str, &str)], move_gen: &MoveGen) {
    ...
    for i in 0..iterations {
        for (j, board) in boards.iter().enumerate() {
            ...
            if i % 5 == 0 && j == 0 {
                ...
                println!(
                    "   Iteration {}: {:.1}% hit rate ({} hits, {} misses)",
                    i + 1,
                    hit_rate * 100.0,
                    hits,
                    misses
    ...
    println!(
        ...
        cache_size,
        max_size,
        (cache_size as f64 / max_size as f64) * 100.0
    );
}
...
fn display_final_statistics() {
    ...
    println!(
        "   🔸 Cache Misses: {} ({:.1}%)",
        misses,
        (misses as f64 / total_requests as f64) * 100.0
    );
    ...
    if cache_size as f64 / max_size as f64 > 0.9 {
        println!(
            "   ⚠️  Cache is nearly full. Consider increasing max_size for better performance."
        );
    }
