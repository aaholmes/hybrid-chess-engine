...
fn show_tt_speedup(positions: &[(&str, &str)], move_gen: &MoveGen) {
    ...
    for (_, fen) in positions {
        ...
        let (_, stats, _) = tactical_mcts_search_with_tt(board, move_gen, config.clone(), &mut tt);
    ...
    for (_, fen) in positions {
        ...
        let (_, stats, _) = tactical_mcts_search_with_tt(
            ...
            move_gen,
            config.clone(),
            &mut shared_transposition_table,
        );
...
fn analyze_mate_cache_efficiency(positions: &[(&str, &str)], move_gen: &MoveGen) {
    ...
    for (i, (name, fen)) in positions.iter().enumerate() {
        ...
        let (best_move, stats, _) =
            tactical_mcts_search_with_tt(board, move_gen, config.clone(), &mut shared_tt);
        ...
        println!(
            "      • Time: {:?}, Iterations: ?{}",
            elapsed, stats.iterations
        );
        println!(
            "      • Mates Found: {}, Best Move: ?{:?}",
            stats.mates_found,
            best_move.is_some()
        );
        println!(
            ...
            hit_rate,
            stats.tt_mate_hits,
            stats.tt_mate_hits + stats.tt_mate_misses
        );
    }
}

fn benchmark_repeated_searches(positions: &[(&str, &str)], move_gen: &MoveGen) {
    ...
    println!(
        "   Running {} rounds on position: ?{}",
        rounds, positions[0].0
    );

    for round in 1..=rounds {
        let start = Instant::now();
        let (_best_move, stats, _) =
            tactical_mcts_search_with_tt(board.clone(), move_gen, config.clone(), &mut shared_tt);
        let elapsed = start.elapsed();
        ...
        println!(
            "   Round {}: {:?}, TT hit rate: {:.1}%, TT size: {} ({} mate)",
