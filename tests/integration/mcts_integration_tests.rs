...
use kingfisher::mcts::tactical_mcts::{
    tactical_mcts_search, tactical_mcts_search_for_training, TacticalMctsConfig,
};
...
fn test_backpropagation_sign_consistency() {
    ...
    for child in root.borrow().children.iter() {
        ...
        if child_ref.visits > 0 {
            ...
            assert!(
                q >= -1.0 && q <= 1.0,
                "Child Q {} outside valid range after backprop",
                q
            );
        }
    }
...
fn test_log_enter_node_contains_fen() {
    ...
    let board = board_from_fen(positions::STARTING);
    let _ = tactical_mcts_search_for_training(board, &move_gen, config);

    let buffer = logger.get_buffer();
    assert!(
        buffer.contains("FEN:"),
        "Debug log should contain FEN output"
    );
}
