...
use kingfisher::mcts::tactical_mcts::{
    tactical_mcts_search, TacticalMctsConfig, TacticalMctsStats,
};
...
fn test_tactical_mcts_returns_legal_move() {
    ...
    let (best_move, stats, _root) = tactical_mcts_search(board.clone(), &move_gen, config);
...
fn test_tactical_mcts_finds_mate_in_1() {
    ...
    assert!(
        stats.mates_found > 0 || stats.tier1_solutions > 0,
...
fn test_tactical_mcts_config_defaults() {
    ...
    assert!(
        config.enable_tier1_gate,
        "Tier 1 should be enabled by default"
    );
    assert!(
        config.mate_search_depth > 0,
        "Mate search depth should be positive"
    );
    assert!(
        config.exploration_constant > 0.0,
        "Exploration constant should be positive"
    );
}
...
fn test_tactical_mcts_tier1_disabled() {
    ...
    assert!(
        best_move.is_some(),
        "Should still return a move with tiers disabled"
    );
    // With tier1 disabled, no tier1 solutions should be found
    assert_eq!(
        stats.tier1_solutions, 0,
        "No tier1 solutions with tier1 disabled"
    );
}
...
fn test_tactical_mcts_stats_populated() {
    ...
    assert!(
        stats.search_time.as_nanos() > 0,
        "search_time should be tracked"
    );
}
...
fn test_tactical_mcts_checkmate_position() {
    ...
    let config = test_config(10);
    let (best_move, _, root) = tactical_mcts_search(board, &move_gen, config);

    // Root should be marked as terminal
    let root_ref = root.borrow();
    assert!(
        root_ref.is_terminal,
        "Checkmate position should be terminal"
    );
    assert_eq!(
        root_ref.terminal_or_mate_value,
        Some(-1.0),
...
fn test_tactical_mcts_time_limit_respected() {
    ...
    assert!(
        elapsed < time_limit + Duration::from_millis(500),
        "Should respect time limit (elapsed: {:?})",
        elapsed
    );
}
...
fn test_q_value_sign_convention() {
    ...
    for child in &root_ref.children {
        ...
        if let Some(mv) = c.action {
            if mv.from == 52 && mv.to == 36 && c.visits > 0 {
                ...
                assert!(
                    q < 0.0,
                    "e7e5 Q should be negative (bad for Black who loses a pawn), got {:.4}",
                    q
                );
            }
        }
    ...
    assert!(
        found_e7e5,
        "e7e5 should have been visited with 50 iterations"
    );
}
...
fn test_koth_gate_value_not_diluted_by_expansion() {
    ...
    let (_best_move, _stats, root) = tactical_mcts_search(board, &move_gen, config);
    ...
    for child in &root_ref.children {
        ...
        if c.terminal_or_mate_value.is_some() {
            ...
            assert!(
                ...
                c.action.map(|m| m.to_uci()).unwrap_or_default(),
                c.children.len()
            );
        }
    }
    assert!(
        gate_resolved_count > 0,
        "Some children should have gate-resolved values in this KOTH position"
    );
}
...
fn test_proven_loss_children_get_minimal_visits() {
    ...
    assert!(
        proven_loss_count > 0,
        "Expected some proven-loss children in this KOTH position"
    );
    assert!(
        non_loss_count > 0,
        "Expected some non-loss children with visits"
    );
}
...
fn test_koth_opponent_on_center_is_terminal() {
    ...
    let root_ref = root.borrow();
    assert!(
        root_ref.terminal_or_mate_value.is_some(),
...
fn test_koth_win_in_1_selected_and_search_aborts() {
    ...
    assert!(
        ...
        "Search should abort early with win-in-1, used {} iterations",
        stats.iterations
    );
}
...
fn test_classical_fallback_material_advantage() {
    ...
    if root_ref.visits > 0 {
        ...
        assert!(
            root_q > 0.0,
            "White up a queen should have positive Q, got {:.4}",
            root_q
        );
    }
}
...
fn test_classical_fallback_material_disadvantage() {
    ...
    if root_ref.visits > 0 {
        ...
        assert!(
            root_q < 0.0,
            "White down a queen should have negative Q, got {:.4}",
            root_q
        );
    }
}
...
fn test_classical_fallback_equal_material() {
    ...
    if root_ref.visits > 0 {
        ...
        assert!(
            root_q.abs() < 0.5,
            "Equal material should have Q near 0, got {:.4}",
            root_q
        );
    }
}
...
fn test_leaf_values_in_tanh_range() {
    ...
    fn check_tree_values(node: &std::rc::Rc<std::cell::RefCell<kingfisher::mcts::node::MctsNode>>) {
        ...
        if let Some(v) = n.nn_value {
            assert!(
                v >= -1.0 && v <= 1.0,
                "nn_value {} outside tanh domain [-1, 1]",
                v
            );
        }
        if let Some(v) = n.terminal_or_mate_value {
            assert!(
                v >= -1.0 && v <= 1.0,
                "terminal_or_mate_value {} outside [-1, 1]",
                v
            );
        }
...
fn test_select_best_move_prefers_koth_win_over_captures() {
    ...
    assert!(
        is_koth,
        "Should prefer KOTH win over rook capture, got {}",
        mv.to_uci()
    );
    ...
    assert!(
        stats.tier1_solutions > 0,
        "Pre-search KOTH gate should fire for win-in-1"
    );
}
...
fn test_training_search_returns_policy() {
    ...
    assert!(
        !result.root_policy.is_empty(),
        "Should return non-empty policy"
    );
    ...
    assert!(
        result.root_value_prediction >= -1.0 && result.root_value_prediction <= 1.0,
        "Root value {} outside [-1, 1]",
        result.root_value_prediction
    );
}
...
fn test_training_search_with_reuse_basic() {
    use kingfisher::mcts::tactical_mcts::{
        reuse_subtree, tactical_mcts_search_for_training_with_reuse,
    };
    ...
    assert!(
        result2.best_move.is_some(),
        "Second search should return a move"
    );
}
...
fn test_dirichlet_noise_modifies_priors() {
    ...
    // Expand the root so it has children
    {
        ...
        for mv in captures.iter().chain(moves.iter()) {
            ...
            if next.is_legal(&move_gen) {
                let child =
                    MctsNode::new_child(std::rc::Rc::downgrade(&root), *mv, next, &move_gen);
                root_ref.children.push(child);
            }
        }
        // Set uniform priors
        let n = root_ref.children.len() as f64;
        let child_moves: Vec<Move> = root_ref
            .children
            .iter()
    ...
    for p in &priors_after {
        assert!(
            *p > 0.0,
            "All priors should remain positive after noise, got {}",
            p
        );
    }
}
...
fn test_tier1_stats_with_mate() {
    ...
    assert!(
        stats.tier1_solutions > 0 || stats.mates_found > 0,
    ...
    assert!(
        stats.tier1_solutions > 0 || stats.nn_saved_by_tier1 > 0,
...
fn test_all_tiers_disabled() {
    ...
    assert!(
        best_move.is_some(),
        "Should still return a move with all tiers disabled"
    );
...
fn test_mock_inference_server_used_in_search() {
    ...
    assert!(
        best_move.is_some(),
        "Should return a move with mock inference server"
    );
    assert!(
        stats.nn_evaluations > 0,
        "Should have NN evaluations when inference server is provided, got {}",
        stats.nn_evaluations
    );
}
...
fn test_mock_uniform_matches_classical() {
    ...
    let (move_classical, _, root_classical) =
        tactical_mcts_search(board.clone(), &move_gen, config_classical);
    ...
    let (move_nn, stats_nn, root_nn) = tactical_mcts_search(board, &move_gen, config_nn);

    // Both should produce the same best move (same evaluation)
    assert_eq!(
        move_classical, move_nn,
    ...
    assert!(
        (q_classical - q_nn).abs() < 0.15,
        "Q-values should be similar: classical={:.4}, nn={:.4}",
        q_classical,
        q_nn
    );
    ...
    assert!(
        stats_nn.nn_evaluations > 0,
        "NN path should track evaluations"
    );
}
...
fn test_biased_mock_changes_evaluation() {
    ...
    let (_, _, root_pos) = tactical_mcts_search(board.clone(), &move_gen, config_pos);
    ...
    let (_, _, root_neg) = tactical_mcts_search(board, &move_gen, config_neg);
    ...
    assert!(
        (q_pos - q_neg).abs() > 0.05,
        "Different biases should produce different Q-values: pos={:.4}, neg={:.4}",
        q_pos,
        q_neg
    );
}
...
fn test_reused_root_with_stale_terminal_value_still_finds_move() {
    ...
    // search gets flagged by mate_search or koth_center_in_3 and is later reused as root.
    {
        ...
        assert!(
            !node.is_terminal,
            "Reused root should not be truly terminal"
        );
...
fn test_enable_material_value_toggle() {
    ...
    assert!(
        q_material > 0.3,
        "With material enabled, white up a queen should have Q > 0.3, got {:.4}",
        q_material
    );

    // Material-disabled classical fallback gives 0.0 for every leaf
    assert!(
        q_no_material.abs() < 0.01,
        "With material disabled and no NN, Q should be ~0.0, got {:.4}",
        q_no_material
    );

    // The two should differ significantly
    assert!(
        (q_material - q_no_material).abs() > 0.2,
