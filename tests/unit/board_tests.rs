...
use kingfisher::piece_types::{BLACK, KING, PAWN, WHITE};
...
fn test_koth_center_squares() {
    ...
    for sq in center_squares {
        assert!(
            (KOTH_CENTER >> sq) & 1 == 1,
            "Square {} should be in KOTH center",
            sq
        );
    }
    assert_eq!(
        KOTH_CENTER.count_ones(),
        4,
        "KOTH center should have exactly 4 squares"
    );
}
