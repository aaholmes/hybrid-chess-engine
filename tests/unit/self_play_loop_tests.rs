...
use kingfisher::mcts::{tactical_mcts_search_for_training_with_reuse, TacticalMctsConfig};
...
fn test_self_play_detects_50_move_draw() {
    ...
    assert!(
        hmc >= 100,
        "Halfmove clock should be >= 100 after 50 full moves, got {}",
        hmc
    );
}
...
fn test_tt_shared_across_moves() {
    ...
    assert!(
        result2.stats.iterations > 0,
        "Second search should complete iterations"
    );
}
...
fn test_halfmove_clock_tracking() {
    ...
    assert_eq!(
        stack.current_state().halfmove_clock(),
        0,
        "Initial halfmove clock"
    );
