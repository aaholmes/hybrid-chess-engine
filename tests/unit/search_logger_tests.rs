...
use kingfisher::mcts::search_logger::{
    GateReason, LogSink, SearchLogger, SelectionReason, Verbosity,
};
...
fn test_gate_reason_descriptions() {
    ...
    let stalemate = GateReason::Terminal {
        is_checkmate: false,
    };
...
fn test_logger_emoji_toggle() {
    let logger_emoji = SearchLogger::buffered(Verbosity::Normal).with_emoji(true);
    ...
    let logger_no_emoji = SearchLogger::buffered(Verbosity::Normal).with_emoji(false);
...
fn test_logger_search_complete() {
    ...
    logger.log_search_complete(Some(Move::new(12, 28, None)), 500, 1000, 2);
