...
fn test_castling_planes() {
    ...
    for plane_idx in 13..17 {
        ...
        assert_eq!(
            sum, 64.0,
            "Castling plane {} should be all 1s when right exists",
            plane_idx
        );
    }
    ...
    for plane_idx in 13..17 {
        ...
        assert_eq!(
            sum, 0.0,
            "Castling plane {} should be all 0s when right missing",
            plane_idx
        );
    }
}
...
fn test_en_passant_plane() {
    ...
    assert_eq!(
        nonzero_count, 1,
        "EP plane should have exactly 1 square marked"
    );
}
