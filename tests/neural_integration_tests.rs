...
fn test_neural_network_integration() {
    ...
    assert!(nn_policy.is_available(), "Neural network should be available when feature is enabled and model exists");
...
fn test_neural_network_stub_behavior() {
    ...
    assert!(nn_policy.predict(&Board::new()).is_none(), "Stub predict should return None.");
    assert_eq!(nn_policy.load("any_path").unwrap_err(), "Neural network feature not enabled (compile with --features neural).".to_string());
}
