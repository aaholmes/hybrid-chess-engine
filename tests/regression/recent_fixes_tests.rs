        ...
        for fen in positions {
            ...
            fn check_tree(node: &Rc<RefCell<MctsNode>>) {
                ...
                if let Some(v) = n.terminal_or_mate_value {
                    assert!(
                        v >= -1.0 && v <= 1.0,
                        "terminal_or_mate_value {} in probability domain!",
                        v
                    );
                }
                ...
                if let Some(v) = n.nn_value {
                    assert!(
                        v >= -1.0 && v <= 1.0,
                        "nn_value {} in probability domain!",
                        v
                    );
                }
                ...
                if n.visits > 0 {
                    ...
                    assert!(
                        q >= -1.0 && q <= 1.0,
                        "average Q {} in probability domain!",
                        q
                    );
                }
...
mod stm_convention {
    ...
    fn test_checkmate_value_is_loss_for_stm() {
        ...
        assert_eq!(
            w_node.borrow().terminal_or_mate_value,
            Some(-1.0),
            "Mated position should be -1.0 for StM (White)"
        );
        assert_eq!(
            b_node.borrow().terminal_or_mate_value,
            Some(-1.0),
...
mod tactical_q_init {
    ...
    fn test_tactical_values_influence_selection() {
...

        {
            ...
            let child1 = MctsNode::new_child(
                Rc::downgrade(&root),
                nxd5,
                root_mut.state.clone(),
                &move_gen,
            );
            let child2 = MctsNode::new_child(
                Rc::downgrade(&root),
                quiet,
                root_mut.state.clone(),
                &move_gen,
        ...
        assert_eq!(
            best_child.unwrap().borrow().action.unwrap(),
            nxd5,
