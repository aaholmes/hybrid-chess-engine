...
**Tier 1 — Safety Gates (exact).** Checks-only mate search (depth 5) and KOTH geometric pruning (can king reach center in 3?). Resolved nodes become **terminal** — never expanded, value is exact. Terminal semantics prevent proven values from being diluted by approximate child evaluations.

**Tier 2 — Quiescence Search (classical tree search).** Every MCTS leaf evaluation runs `forced_material_balance()`: a material-only alpha-beta Q-search (depth 8) that resolves all forced captures and promotions. This produces deltaM — the true material balance after tactical dust settles. A position may look equal (P=P) but after forced exchanges be +3. No neural network can easily replicate this because it requires a classical tree search over variable-depth exchange sequences. deltaM feeds directly into the value function (see below). Additionally, captures are visited in MVV-LVA order on their first visit (PxQ before QxP), though this is a minor optimization.
...
- **K head**: Operates on raw input (independent of backbone). 12 handcrafted scalar features (pawn count, piece counts, queen presence, castling rights, king rank, bishop colors) + two 5x5 king-centered patches compressed via FC(300→32). Combined → FC(76→32→1). Output: softplus(x)/(2 ln 2), so k=0.5 at init.
