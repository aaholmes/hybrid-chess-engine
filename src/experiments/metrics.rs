...
impl SearchMetrics {
    ...
    pub fn nn_call_reduction_percent(&self) -> f64 {
        let potential_calls =
            self.nn_evaluations + self.nn_calls_saved_tier1 + self.nn_calls_saved_tier2;
...
impl AggregatedMetrics {
    pub fn from_search_metrics(config_name: &str, metrics: &[SearchMetrics]) -> Self {
        ...
        // Calculate means
        let mean_iterations = metrics
            .iter()
            .map(|m| m.total_iterations as f64)
            .sum::<f64>()
            / n;
        let mean_nodes = metrics.iter().map(|m| m.nodes_expanded as f64).sum::<f64>() / n;
        let mean_time_ms = metrics
            .iter()
            .map(|m| m.search_time.as_millis() as f64)
            .sum::<f64>()
            / n;
        let mean_nn_reduction = metrics
            .iter()
            .map(|m| m.nn_call_reduction_percent())
            .sum::<f64>()
            / n;

        // Calculate standard deviations
        let std_iterations = (metrics
            .iter()
            .map(|m| (m.total_iterations as f64 - mean_iterations).powi(2))
            .sum::<f64>()
            / n)
            .sqrt();
        let std_nn_reduction = (metrics
            .iter()
