...
impl MctsNode {
    ...
    pub fn puct_value(
        ...
        let exploration_term = exploration_constant * prior_p * (parent_visits as f64).sqrt()
    ...
    pub fn select_best_explored_child(
        &self,
        _move_gen: &MoveGen,
        exploration_constant: f64,
    ) -> Rc<RefCell<MctsNode>> {
        ...
        self.children
            ...
            .max_by(|a, b| {
                let puct_a =
                    a.borrow()
                        .puct_value(parent_visits, parent_num_legal, exploration_constant);
                let puct_b =
                    b.borrow()
                        .puct_value(parent_visits, parent_num_legal, exploration_constant);
                puct_a
    ...
    pub fn categorize_and_store_moves(&mut self, move_gen: &MoveGen) {
        ...
        for mv in &legal_moves {
            ...
            let is_capture = (self.state.pieces_occ[opponent_color] & (1u64 << mv.to)) != 0
                || mv.is_en_passant();
    ...
    fn recursive_dot(
        ...
        let label = format!(
            "{{ {} | {} | N: {} | Q: {:.2} | V: {} }}",
            move_str, tier_label, self.visits, q_value, eval_str
        );
        ...
        for child in &self.children {
            ...
            if let Some(mv) = child_ref.action {
                let mg = MoveGen::new();
                if self.state.get_piece(mv.from).is_none()
                    || !self.state.apply_move_to_board(mv).is_legal(&mg)
                {
                    continue;
                }
            }
