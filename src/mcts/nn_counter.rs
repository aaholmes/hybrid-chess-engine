...
impl EfficiencyComparison {
    ...
    pub fn print_comparison(&self) {
        ...
        println!(
            "  NN calls: {} / {} positions",
            self.tactical_mcts_calls, self.tactical_mcts_positions
        );
        println!(
            "  Efficiency: {:.3} calls per position",
            self.tactical_efficiency()
        );

        println!("\nClassical MCTS:");
        println!(
            "  NN calls: {} / {} positions",
            self.classical_mcts_calls, self.classical_mcts_positions
        );
        println!(
            "  Efficiency: {:.3} calls per position",
            self.classical_efficiency()
        );

        println!(
            "\nImprovement: {:.1}% reduction in NN calls",
