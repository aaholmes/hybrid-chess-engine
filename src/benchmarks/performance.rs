...
impl PerformanceComparison {
    ...
    pub fn print_comparison(&self) {
        ...
        for (engine_name, summary) in &self.results {
            let speed_metric = if summary.average_time.as_millis() > 0 {
                format!(
                    "{:.0} n/s",
                    summary.total_nodes as f64
                        / summary.average_time.as_secs_f64()
                        / summary.total_positions as f64
                )
            } else {
                "N/A".to_string()
            };
    ...
    fn analyze_results(&self) {
        ...
        println!(
            "   🎯 Highest Accuracy: {} ({:.1}%)",
            best_accuracy.0, best_accuracy.1.mate_accuracy
        );
        println!(
            "   ⚡ Fastest: {} ({:.1}ms avg)",
            fastest.0,
            fastest.1.average_time.as_millis()
        );
        println!(
            "   🏆 Most Efficient: {} ({:.2} acc/ms)",
            most_efficient.0,
        ...
        if let Some(baseline) = self
            .results
            .iter()
            .find(|(name, _)| name.contains("AlphaBeta"))
        {
            for (engine_name, summary) in &self.results {
                if engine_name != &baseline.0 {
                    ...
                    let speed_improvement = if summary.average_time < baseline.1.average_time {
                        let speedup = baseline.1.average_time.as_millis() as f64
                            / summary.average_time.as_millis() as f64;
                        format!("{:.1}x faster", speedup)
                    } else {
                        let slowdown = summary.average_time.as_millis() as f64
                            / baseline.1.average_time.as_millis() as f64;
                        format!("{:.1}x slower", slowdown)
                    };

                    println!(
                        "   📈 {} vs {}: {:.1}% accuracy improvement, {}",
...
pub fn run_performance_comparison(time_limit_per_position: Duration) -> PerformanceComparison {
    ...
    let ab_results =
        run_tactical_benchmark(&mut simple_agent, "AlphaBeta", time_limit_per_position);
    ...
    let mcts_results = run_tactical_benchmark(
        &mut humanlike_agent,
        "MateSearchFirst-MCTS",
        time_limit_per_position,
    );
...
pub fn mate_speed_benchmark() {
    ...
    for time_limit in time_limits {
        ...
        let mcts_results =
            run_tactical_benchmark(&mut humanlike_agent, "MateSearchFirst", time_limit);
        ...
        if mcts_summary.mate_accuracy > ab_summary.mate_accuracy {
            ...
            println!(
                "   🎯 MateSearchFirst advantage: +{:.1}% accuracy",
                improvement
            );
        }
    }
