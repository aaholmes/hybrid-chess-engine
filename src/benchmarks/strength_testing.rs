...
impl StrengthTester {
    ...
    pub fn run_comprehensive_test(&mut self) -> StrengthTestResults {
        ...
        println!(
            "📊 Testing {} positions across {} engine variants",
            test_positions.len(),
            5
        );
    ...
    fn test_engine_variant(
        ...
        for (i, position) in positions.iter().enumerate() {
            ...
            let (best_move, evaluation, nodes) =
                self.analyze_position_with_engine(engine, &position.board);
            ...
            let move_quality =
                self.calculate_move_quality(&position.board, best_move, position.category);
    ...
    fn analyze_position_with_engine(
        &mut self,
        engine: EngineVariant,
        board: &Board,
    ) -> (Option<Move>, Option<i32>, Option<u64>) {
            ...
            EngineVariant::AlphaBeta => {
                ...
                for mv in captures.iter().chain(non_captures.iter()) {
                    ...
                    if new_board.is_legal(&self.move_gen) {
                        ...
                        if (board.w_to_move && eval > best_eval)
                            || (!board.w_to_move && eval < best_eval)
                        {
                            best_eval = eval;
                            best_move = Some(*mv);
                        }
    ...
    fn mate_priority_evaluation_search(&self, board: &Board, time_limit: Duration) -> Option<Move> {
        ...
        for mv in captures.iter().chain(non_captures.iter()) {
            ...
            if new_board.is_legal(&self.move_gen) {
                ...
                let (opp_captures, opp_non_captures) =
                    self.move_gen.gen_pseudo_legal_moves(&new_board);
                let has_legal_moves =
                    opp_captures
                        .iter()
                        .chain(opp_non_captures.iter())
                        .any(|opp_mv| {
    ...
    fn calculate_move_quality(
        &self,
        board: &Board,
        chosen_move: Option<Move>,
        category: PositionCategory,
    ) -> f64 {
        let Some(mv) = chosen_move else {
            return 0.0;
        };
        ...
        let eval_diff = if board.w_to_move {
            // White to move
    ...
    fn calculate_engine_summary(
        ...
        let average_time_ms =
            results.iter().map(|r| r.time_taken_ms as f64).sum::<f64>() / total_positions as f64;
        let average_move_quality =
            results.iter().map(|r| r.move_quality_score).sum::<f64>() / total_positions as f64;
    ...
    fn calculate_overall_comparison(
        &self,
        summaries: &HashMap<EngineVariant, EngineSummary>,
    ) -> OverallComparison {
        // Create ranking based on combined score (accuracy + move quality)
        let mut ranking: Vec<(EngineVariant, f64)> = summaries
            .iter()
            .map(|(engine, summary)| {
                let combined_score = (summary.accuracy_percentage / 100.0) * 0.6
                    + summary.average_move_quality * 0.4;
        ...
        let best_engine = ranking
            .first()
            .map(|(engine, _)| *engine)
            .unwrap_or(EngineVariant::AlphaBeta);

        // Calculate improvements
        let ab_score = summaries
            .get(&EngineVariant::AlphaBeta)
            .map(|s| s.average_move_quality)
            .unwrap_or(0.5);
        let mcts_classical_score = summaries
            .get(&EngineVariant::MctsClassical)
            .map(|s| s.average_move_quality)
            .unwrap_or(0.5);
        let mcts_mate_score = summaries
            .get(&EngineVariant::MctsMatePriority)
            .map(|s| s.average_move_quality)
            .unwrap_or(0.5);
        let mcts_neural_score = summaries
            .get(&EngineVariant::MctsNeuralNet)
            .map(|s| s.average_move_quality)
            .unwrap_or(0.5);
        let mcts_complete_score = summaries
            .get(&EngineVariant::MctsComplete)
            .map(|s| s.average_move_quality)
            .unwrap_or(0.5);

        let mate_search_improvement =
            ((mcts_mate_score - mcts_classical_score) / mcts_classical_score) * 100.0;
        let neural_net_improvement =
            ((mcts_neural_score - mcts_classical_score) / mcts_classical_score) * 100.0;
    ...
    fn print_results_summary(&self, results: &StrengthTestResults) {
        ...
        println!(
            "  Mate-Search-First:  {:+.1}%",
            results.overall_comparison.mate_search_improvement
        );
        println!(
            "  Neural Network:     {:+.1}%",
            results.overall_comparison.neural_net_improvement
        );
        println!(
            "  Combined Approach:  {:+.1}%",
        ...
        for engine in &[
            ...
            if let Some(summary) = results.engine_summaries.get(engine) {
                ...
                println!(
                    "     Accuracy:      {:.1}% ({}/{})",
                    summary.accuracy_percentage, summary.correct_moves, summary.total_positions
                );
        ...
        if results.overall_comparison.combined_improvement > 0.0 {
            println!(
                "  ✅ Our mate-search-first + neural network approach shows {:.1}% improvement!",
                results.overall_comparison.combined_improvement
            );
        } else {
            println!(
                "  ⚠️  Need more tuning - combined approach shows {:.1}% change",
                results.overall_comparison.combined_improvement
            );
        }
    }
