...
impl EloCalculator {
    ...
    pub fn estimate_performance_ratings(
        &self,
        results: &[PositionResult],
    ) -> HashMap<EngineVariant, EloEstimate> {
    ...
    fn calculate_engine_rating(
        ...
        let average_quality =
            results.iter().map(|r| r.move_quality_score).sum::<f64>() / total_positions;
        let average_time =
            results.iter().map(|r| r.time_taken_ms as f64).sum::<f64>() / total_positions;
    ...
    pub fn compare_engines(
        &self,
        engine_a: &EloEstimate,
        engine_b: &EloEstimate,
    ) -> EngineComparison {
    ...
    pub fn generate_elo_report(
        &self,
        estimates: &HashMap<EngineVariant, EloEstimate>,
    ) -> EloReport {
...
impl EloReport {
    ...
    pub fn print_report(&self) {
        ...
        for (i, estimate) in self.rankings.iter().enumerate() {
            ...
            println!(
                "  {}. {} - {:.0} Elo ({:+.0} vs baseline) [confidence: {:.1}%]",
                i + 1,
                estimate.engine,
        ...
        for comparison in &self.comparisons {
            if matches!(
                ...
                ComparisonSignificance::Significant | ComparisonSignificance::Moderate
            ) {
                println!(
                    "  {} vs {}: {:+.0} Elo ({:?})",
        ...
        for estimate in &self.rankings {
            ...
            println!(
                "     Move Quality: {:.3} ({:+.0} Elo)",
                estimate.average_quality, estimate.rating_components.quality_component
            );
            println!(
                "     Speed: {:.0}ms ({:+.0} Elo)",
                estimate.average_time_ms, estimate.rating_components.time_component
            );
            println!(
                "     Confidence: {:.1}% ({} positions)",
                estimate.confidence * 100.0,
        ...
        if self.overall_improvement > 50.0 {
            println!(
                "  ✅ Significant improvement: {:+.0} Elo gain from innovations!",
                self.overall_improvement
            );
        } else if self.overall_improvement > 0.0 {
            println!(
                "  📈 Moderate improvement: {:+.0} Elo gain",
                self.overall_improvement
            );
        } else {
            println!(
                "  ⚠️  No significant improvement detected ({:+.0} Elo)",
                self.overall_improvement
            );
        }
