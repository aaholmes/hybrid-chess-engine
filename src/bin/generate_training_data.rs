...
use kingfisher::training::{ParsedGame, TrainingDataGenerator};
    ...
    match TrainingDataGenerator::load_from_csv(csv_path) {
        Ok(loaded_positions) => {
            println!(
                ...
                loaded_positions.len()
            );

            // Show some statistics
            let white_wins = loaded_positions
                .iter()
                .filter(|p| p.game_result > 0.7)
                .count();
            let draws = loaded_positions
                .iter()
                .filter(|p| (p.game_result - 0.5).abs() < 0.2)
                .count();
            let black_wins = loaded_positions
                .iter()
                .filter(|p| p.game_result < 0.3)
                .count();
            ...
            println!(
                "   White wins: {} ({:.1}%)",
                white_wins,
                white_wins as f64 / loaded_positions.len() as f64 * 100.0
            );
            println!(
                "   Draws: {} ({:.1}%)",
                draws,
                draws as f64 / loaded_positions.len() as f64 * 100.0
            );
            println!(
                "   Black wins: {} ({:.1}%)",
                black_wins,
                black_wins as f64 / loaded_positions.len() as f64 * 100.0
            );
...
fn create_sample_games() -> Vec<ParsedGame> {
    ...
    game1
        .metadata
        .insert("Event".to_string(), "Sample Game 1".to_string());
    ...
    game2
        .metadata
        .insert("Event".to_string(), "Sample Game 2".to_string());
    ...
    game3
        .metadata
        .insert("Event".to_string(), "Sample Game 3".to_string());
