...
use kingfisher::benchmarks::elo_tournament::{EloTournament, TournamentConfig, TournamentEngine};
use std::env;
use std::fs::File;
use std::io::Write;
...
fn main() {
    ...
    let output_csv =
        parse_string_arg(&args, "--output").unwrap_or_else(|| "tournament_results.csv".to_string());
